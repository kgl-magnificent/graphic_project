...
class MyItem2(QtWidgets.QGraphicsItem):

    def __init__(self, id):
        super().__init__()
        self.id = id
        self.id2 = 0
        self.flag_big_cicrle = 0
        self.flag_small_circle = 0
        self.addText = 0
        self.flag_select = 0
    ...
    #метод для рисования маленького круга
    def paint_small(self):
        self.flag_big_cicrle = 0
        self.flag_small_circle = 1
        self._brush = QtGui.QBrush(QtCore.Qt.green)
        self.rectF = QtCore.QRectF(-18, -18, 35, 35)

    # метод для рисования большого круга
    def paint_big(self):
        self.flag_big_cicrle = 1
        self.flag_small_circle = 0
    ...
    def paint(self, painter, option=None, style=None, widget=None):
        ...
        if self.flag_big_cicrle == 1:
            ...
            if self.addText == 1:
                ...
                painter.drawText(-len(str(self.text))*4, -55, str(self.text))



        # рисуем маленький зеленый круг
        if self.flag_small_circle == 1:
            painter.setBrush(self._brush)
            ...
            if self.addText == 1:
                ...
                if len(self.text) >= 5:
                    ...
                    self.setToolTip(str(self.text))
                    self.sam_text = self.text[:4] + ".."
...
class MyMainWindow(QtWidgets.QMainWindow):
    ...
    def initObj(self):
        for key in self.dict_obj_name:
            self.dict_of_obj[key] = MyItem2(key)
            #self.dict_of_obj[key].paint_big()
            #self.dict_of_obj[key].setPos(100, 100+key*20)
            self.scene.addItem(self.dict_of_obj[key])

        # print("list_of_obj = ", self.dict_of_obj[3].id)
        # print("list_of_obj = ", self.dict_of_obj[3].id2)
    ...
    def initUI(self, id_new_obj = -1):

        if id_new_obj == -1:
            id = 0 #id сцены
            self.locationX = -150
            self.locationY = -100
            if id in self.dict_with_elem_conn.keys(): #id = 0 это сцена, тут мы смотрим какие объекты будут на нее добалвены

                self.main_circles = self.dict_with_elem_conn.get(0) # формируем список в который будут добавлены объекты сцены
                for main_circle_in_scene in self.main_circles: #проходим по этому списку
                    self.dict_of_obj.get(main_circle_in_scene).paint_big()
                    self.dict_of_obj.get(main_circle_in_scene).setPos(self.locationX, self.locationY)
                    self.dict_of_obj.get(main_circle_in_scene).add_text(self.dict_obj_name.get(main_circle_in_scene))
                    ...
                    count = len(self.dict_with_elem_conn.get(main_circle_in_scene))
                    ...
                    if count > 1:
                        ...
                        for i in range(count):
                            ...
                            l.append([x, y])
                    index = 0
                    for i in self.dict_with_elem_conn.get(main_circle_in_scene):
                        self.dict_of_obj.get(i).paint_small()
                        self.dict_of_obj.get(i).add_text(self.dict_obj_name.get(i))
                        self.dict_of_obj.get(i).setPos(l[index][0], l[index][1])
                        self.dict_of_obj.get(i).setParentItem(self.dict_of_obj.get(main_circle_in_scene))
                        index = index + 1

        if id_new_obj != -1:
            self.dict_of_obj.get(id_new_obj).paint_big()
            self.dict_of_obj.get(id_new_obj).setPos(0, 0)

    def initUI2(self, flag = -1):
